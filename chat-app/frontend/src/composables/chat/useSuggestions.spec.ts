...
describe('useSuggestions - AI 建議回覆測試', () => {
  let useSuggestions: any;
  let conversationUtils: any;
  let mockMessages: Ref<Message[]>;
  let mockPartner: Ref<Partner>;
  let mockFirebaseAuth: FirebaseAuth;
  let mockCurrentUserId: Ref<string>;
  ...
  describe('loadSuggestions', () => {
    ...
    it('應該在沒有 matchId 時返回預設選項', async () => {
      mockPartner.value = { id: '', display_name: '' }; // 空 matchId
  ...
  describe('邊界情況', () => {
    ...
    it('應該處理 currentUserId 為 null', async () => {
      mockCurrentUserId.value = null as any;
