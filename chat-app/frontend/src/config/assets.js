 ...
 */
export const COIN_ICON_PATH = '/icons/wallet-coin.png';
 ...
 * 擴展資產卡片配置（添加 UI 組件）
 */
export const ASSET_CARDS = Object.fromEntries(
  Object.entries(ASSET_CARDS_BASE_CONFIG).map(([key, card]) => [
    key,
    {
      ...card,
 ...
 * 擴展道具配置（添加 UI 組件）
 */
export const ASSET_POTIONS = Object.fromEntries(
  Object.entries(POTIONS_BASE_CONFIG).map(([key, potion]) => [
    key,
    {
      ...potion,
 ...
 * 根據 assetKey 獲取資產配置
 */
export const getAssetByKey = (assetKey) => {
  // 查找卡片
  const card = Object.values(ASSET_CARDS).find(c => c.assetKey === assetKey);
  if (card) return card;

  // 查找道具
  const potion = Object.values(ASSET_POTIONS).find(p => p.assetKey === assetKey);
 ...
 * 根據商城分類 ID 獲取資產配置
 */
export const getAssetByShopCategory = (categoryId) => {
  // 查找卡片
  const card = Object.values(ASSET_CARDS).find(c => c.shopCategory === categoryId);
  if (card) return card;

  // 查找道具
  const potion = Object.values(ASSET_POTIONS).find(p => p.shopCategory === categoryId);
  if (potion) return potion;
 ...
 * 根據圖標顏色 ID 獲取樣式
 */
export const getIconColor = (colorId) => {
  return ICON_COLORS[colorId] || ICON_COLORS.coins;
};
