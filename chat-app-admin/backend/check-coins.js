...
async function checkUserCoins() {
  try {
    ...
    console.log('  "wallet.balance" (錯誤欄位名):', data['wallet.balance'] ?? '❌ 不存在');
    ...
    if (uniqueValues.size > 1) {
      ...
      console.log('     這可能導致前後台顯示不同的金幣數量！');
    }

    console.log('\n📝 修復建議：');
    if (data['wallet.balance'] !== undefined || !data.wallet) {
      console.log('  需要修復 Firestore 數據結構');
    }
