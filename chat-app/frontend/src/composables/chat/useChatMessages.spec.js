...
describe('useChatMessages - 聊天消息管理測試', () => {
  ...
  beforeEach(async () => {
    ...
    useUserProfile = userProfile.useUserProfile;
    useFirebaseAuth = firebaseAuth.useFirebaseAuth;
  ...
  describe('重試機制', () => {
    ...
    it('應該在達到最大重試次數後標記為失敗', async () => {
      ...
      // 應該標記為失敗
      const failedMsg = chat.messages.value.find(m => m.text === 'Test');
  ...
  describe('retryFailedMessage', () => {
    it('應該成功重試失敗的消息', async () => {
      ...
      const retriedMsg = chat.messages.value.find(m => m.id === 'failed-msg-1');
  ...
  describe('resetConversation', () => {
    ...
    it('應該處理重置錯誤', async () => {
      global.fetch = vi.fn(() => Promise.reject(new Error('Reset failed')));
  ...
  describe('認證權杖處理', () => {
    ...
    it('應該在無法獲取 token 時標記消息為失敗', async () => {
      ...
      // 所有重試耗盡後，消息應該標記為失敗
      const msg = chat.messages.value.find(m => m.text === 'Test');
