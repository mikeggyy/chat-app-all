...
type StoreType = Storage | Map<string, unknown>;

const MAX_CACHED_MESSAGES = 200;

const memoryHistoryStore = new Map<string, Message[]>();
const memoryPendingStore = new Map<string, Message[]>();

let storageAvailable: boolean | undefined = undefined;
...
const buildKey = (prefix: string, userId: string, characterId: string): string => {
...
const readFromStore = (store: StoreType, key: string): Message[] | null => {
  if (store instanceof Map) {
    return (store.get(key) as Message[] | undefined) ?? null;
  }
...
const writeToStore = (store: StoreType, key: string, value: Message[] | null): void => {
  if (store instanceof Map) {
    if (value === null) {
      store.delete(key);
    } else {
...
const clipMessages = (messages: Message[] | null | undefined = []): Message[] => {
...
export const readCachedHistory = (userId: string, characterId: string): Message[] => {
...
export const writeCachedHistory = (userId: string, characterId: string, messages: Message[]): void => {
  if (!userId || !characterId) return;
  const key = buildKey("history", userId, characterId);
  const storage = getStorage();
  const store = storage ?? memoryHistoryStore;
...
export const appendCachedHistory = (userId: string, characterId: string, messages: Message | Message[]): void => {
  if (!userId || !characterId) return;
...
export const readPendingMessages = (userId: string, characterId: string): Message[] => {
  if (!userId || !characterId) return [];
...
const uniqueById = (messages: Message[]): Message[] => {
...
export const enqueuePendingMessages = (userId: string, characterId: string, messages: Message | Message[]): void => {
  if (!userId || !characterId) return;
  const key = buildKey("pending", userId, characterId);
...
export const removePendingMessagesById = (userId: string, characterId: string, ids: string | string[] = []): void => {
  if (!userId || !characterId) return;
  const key = buildKey("pending", userId, characterId);
  const current = readPendingMessages(userId, characterId);
...
export const clearPendingMessages = (userId: string, characterId: string): void => {
  if (!userId || !characterId) return;
  const key = buildKey("pending", userId, characterId);
  const storage = getStorage();
  const store = storage ?? memoryPendingStore;
 ...
 * 返回值以 KB 為單位
 */
export const estimateLocalStorageUsage = (): StorageUsage => {
  const storage = getStorage();
  if (!storage) {
