...
/**
 * useVoiceLoading 返回值類型
 */
export interface UseVoiceLoadingReturn {
  voicePresets: ComputedRef<VoicePreset[]>;
  voicePresetsFromAPI: Ref<VoicePreset[]>;
  fallbackVoicePresets: VoicePreset[];
  isLoadingVoices: Ref<boolean>;
  voicesLoadError: Ref<string | null>;
  loadVoicesFromAPI: () => Promise<void>;
  resolvePreviewUrl: (voiceId: string | null | undefined) => string;
}
 ...
 */
export function useVoiceLoading(): UseVoiceLoadingReturn {
  // 從 API 動態載入的語音列表
  const voicePresetsFromAPI = ref<VoicePreset[]>([]);
  const isLoadingVoices = ref<boolean>(false);
  const voicesLoadError = ref<string | null>(null);
   ...
   */
  const voicePresets = computed<VoicePreset[]>(() =>
   ...
   * 從後端 API 載入語音列表
   */
  const loadVoicesFromAPI = async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : '載入語音失敗';
      logger.error('[Voices] 載入語音失敗，使用備用語音列表:', error);
      voicesLoadError.value = errorMessage;
    } finally {
   ...
   * 解析預覽 URL
   */
  const resolvePreviewUrl = (voiceId: string | null | undefined): string => {
    if (!voiceId) {
      return '';
    }
    const preset = voicePresets.value.find((item: VoicePreset) => item.id === voiceId);
